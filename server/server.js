...
app.get('/', async (req, res) => {
  try {
    ...
    const uitgebreidPath = path.join(__dirname, '../client/public/uitgebreid.json');
...
app.get('/verhaal/:naam', async (req, res) => {
  try {
    const naam = decodeURIComponent(req.params.naam); // Decodeert naam uit URL.
...
app.get('/:straatnaam', async (req, res) => {
  ...
  try {
    ...
    const huidigAdres = adressen.data.filter(adres => adres.street?.trim() === straatnaam); // Filter op straatnaam

    await Promise.all(huidigAdres.map(async (adres) => {
      const personen = adres.person;
      if (Array.isArray(personen) && personen.length > 0) {
        // Haalt info op over elke persoon
    ...
    res.render('street', {
      huidigAdres,
      straatnaam
    }); // Render de straatpagina met adressen.
